...
export async function POST(request: Request) {
  ...
  const timeoutMs = hasImage ? IMAGE_AI_TIMEOUT_MS : AI_TIMEOUT_MS;
  ...
  try {
    ...
    const rawModelJson = parseModelJson(extractArkText(ark));

    if (!rawModelJson) {
      return NextResponse.json(getFallbackResult(fallbackTitle, false));
    }

    return NextResponse.json(normalizeParsedResult(rawModelJson, fallbackTitle, hasImage ? null : text));
  } catch (error) {
